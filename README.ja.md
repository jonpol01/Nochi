...
Nochiは選択された言語ペアのオンデバイス音声・翻訳モデルが未インストールの場合を自動検出し、ダウンロードへ誘導します。

### 1. マイクボタンのオレンジバッジ

ソース→ターゲットのペアに設定が必要な場合、マイクアイコンに小さなドットが表示されます。

<div align="center">
<img src="assets/screenshots/overlay-warning.png" width="100%" alt="オレンジの警告バッジ付きオーバーレイ" />
</div>

### 2. セットアップ必要のダイアログ

マイクボタンをクリックすると、黙って失敗する代わりに親切なダイアログが表示されます。システム設定へ直接ジャンプするか、そのまま開始（サーバーモード）するかを選べます。

<div align="center">
<img src="assets/screenshots/setup-alert.png" width="360" alt="セットアップ必要ダイアログ" />
</div>

### 3. システム設定への直接リンク

アプリが言語パックをダウンロードできる正確なシステム設定パネルを開きます。

<div align="center">
<img src="assets/screenshots/language-region.png" width="520" alt="システム設定 > 言語と地域" />
</div>

一般的なペア（JA↔EN、EN↔ES、EN↔ZHなど）はmacOSに事前インストール済みのため、このフローは発生しません。上記の`JA → TH`のようにダウンロードが必要なペアでのみ表示されます。
